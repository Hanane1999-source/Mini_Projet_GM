...
function DemarrerUnNouveauTexte() {
  TexteEnCoursEdition = {                                  
    text: '',                             
    x: 50,
    y: 50,                                                 //nouveau texte vide sur une position de 50;50               
...
textInput.addEventListener("input", () => {
  if (!TexteEnCoursEdition) return;                      //éviter une erreur si aucun mèmes n’est sélectionné 
...
// === Drag & Drop (glisser-déposer): détecter le texte sélectionné ===
function VerifierPositionTexte(x, y) {
  ...
  for (let i = texts.length - 1; i >= 0; i--) {
    ...
    if (!obj || !obj.text) continue;                                                    //sauté l’objet s’il est nul ou ne contient pas de texte.

    const width = ctx.measureText(obj.text).width;                                      //mesuré la largeur du texte avec le contexte du canvas (ctx)
    const height = parseInt(obj.fontSize, 10);                                          //récupèré la hauteur du texte en analysant la taille de police (fontSize) (approximatif).

    if (x > obj.x && x < obj.x + width && y > obj.y - height && y < obj.y) {            //vérifier si (x, y) est à l'intérieur du rectangle occupé par le texte.
  ...
  if (TexteEnCoursEdition && TexteEnCoursEdition.text) {                                 //vérifier s’il y a un texte en cours d’édition == Mesure les dimensions de ce texte.
    ...
    if (x > TexteEnCoursEdition.x && x < TexteEnCoursEdition.x + width &&                         
      ...
      IndexDuTexteSelectionne = texts.length;                                            //lui donne un index spécial (en dehors du tableau)
      positionRelativeX = x - TexteEnCoursEdition.x;                                     //préparer les coordonnées pour le déplacement
...
function appliquerDeplacement(x, y) {                                                       
  if (IndexDuTexteSelectionne === null) return;                                           //aucun texte n’a été sélectionné, on ne fait rien
  ...
  texteADeplacer.x = x - positionRelativeX;                                              //appliquer le nouvel emplacement du texte, en prenant en compte le décalage relatif lors du clic initial.
  texteADeplacer.y = y - positionRelativeY;
  DessinerMeme();                                                                        //redessiner tout le canvas avec le texte déplacé.
}

// === Gestion des événements souris ===
canvas.addEventListener("mousedown", e => {                                              //calculer la position du clic relative au canvas et on appelle la fonction VerifierPositionTexte avec avec ces coordonnées
  const rect = canvas.getBoundingClientRect();
  VerifierPositionTexte(e.clientX - rect.left, e.clientY - rect.top);
});
canvas.addEventListener("mousemove", e => {
  if (IndexDuTexteSelectionne !== null) {                                               //vérifier qu’un texte est en cours de déplacement 
    const rect = canvas.getBoundingClientRect();                                        //récupèrer la position du canvas dans la fenêtre (utile pour ajuster la position du curseur).
    appliquerDeplacement(e.clientX - rect.left, e.clientY - rect.top);                  //appliquer le déplacement du texte à la nouvelle position du curseur.
  }
});
...
canvas.addEventListener("mouseleave", () => IndexDuTexteSelectionne = null);           //si la souris sort du canvas pendant le déplacement, on annule aussi le glisser-déposer.

// === Événements tactiles ===
canvas.addEventListener("touchstart", e => {                                                                   
  e.preventDefault();                                                                  //pour empêcher le zoom/pan automatique du navigateur (scrolling ou zoom tactile).
  const rect = canvas.getBoundingClientRect();                                         //calculer la position du la touche tactile relative au canvas et on appelle la fonction VerifierPositionTexte avec avec ces coordonnées
